...
TwistedCultist::TwistedCultist(cocos2d::Vector<cocos2d::Node*>& player, BehaviorTree* aiTree):
	Enemy(player,aiTree)
{
	pos_ = { 980,200 };
...
TwistedCultist* TwistedCultist::CreateTwistedCultist(cocos2d::Vector<cocos2d::Node*>& player, BehaviorTree* aiTree)
{
	TwistedCultist* pRet = new(std::nothrow) TwistedCultist(player,aiTree);
	if (pRet && pRet->init())
	{
		pRet->autorelease();
		return pRet;
